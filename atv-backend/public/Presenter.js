  ...
  load: function(event) {
    ...
    if (templateURL) {
      ...
      /*
             ...
             */
      var toLoad = templateURL;
      resourceLoader.loadResource(toLoad, function(resource) {
        if (resource) {
          /*
      ...
      if (action == "dismiss" && event.type == "select") {
        navigationDocument.dismissModal();
      }

      if (url && event.type == "select") {
        var player = new Player();
        var playList = new Playlist();
        var item = new MediaItem("video", url);
        playList.push(item);
